...
interface PortfolioProps {
  ...
  users: User[];
  themes: Theme[];
  onSelectBet: (bet: Bet, taskId?: string) => void;
  onNewBet: () => void;
...
const Portfolio: React.FC<PortfolioProps> = ({ bets, users, themes, onSelectBet, onNewBet, onDeleteBet, onArchiveBet, currentUser }) => {
  ...
  const filteredBets = bets.filter(bet => {
    ...
    const matchesStage = filterStage === 'all' || bet.stage === filterStage;
    const matchesOwner = filterOwner === 'all' || bet.owner_user_ids.includes(filterOwner);
    return matchesSearch && matchesTheme && matchesStage && matchesOwner;
  });
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <GenericFilterBar
        ...
        selectedStatus={filterStage}
        onStatusChange={setFilterStage}
                          ...
                          {bet.actions.map(action => {
                            ...
                            return (
                              <div
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  onSelectBet(bet, action.id);
                                }}
                                className="flex items-center gap-4 bg-slate-950/50 p-3 rounded-lg border border-slate-800/50 hover:border-slate-700 transition-colors cursor-pointer"
                              >
                                <div className="flex-1">
                                  ...
                                  <div className="flex items-center gap-2 mt-1">
                                    ...
                                    {assignee && (
                                      <div className="flex items-center gap-1.5 bg-slate-900 pr-2 pl-0.5 py-0.5 rounded-full border border-slate-800">
      ...
      {betToDelete && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <header className="px-6 py-4 border-b border-slate-800 flex justify-between items-center bg-rose-950/20">
            ...
            <div className="p-8 space-y-6 text-center">
              <p className="text-sm text-slate-300 leading-relaxed">
                Are you sure you want to delete <strong className="text-white">"{betToDelete.title}"</strong>?
              ...
              <div className="flex gap-3">
                <button
                  onClick={() => setBetToDelete(null)}
                  className="flex-1 py-3 border border-slate-700 text-slate-300 font-bold rounded-xl hover:bg-slate-800 hover:text-white transition-colors"
                >
                  Cancel
                </button>
                <button

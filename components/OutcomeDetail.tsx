...
import { Outcome1Y, Measure, User, Status } from '../types';
...
const OutcomeDetail: React.FC<OutcomeDetailProps> = ({ outcome, measures, currentUser, onClose, onUpdate, onAddMeasure, onUpdateMeasure, onDeleteMeasure }) => {
  ...
  const handleAddSuggested = (s: any) => {
    const newMeasure: Measure = {
      ...
      source_type: 'Manual',
      target: s.threshold,
      thresholds: { green_above: 0 } // Default
    };
      ...
      <div className="w-full max-w-2xl bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
        <header className="p-6 border-b border-slate-100 flex justify-between items-center bg-white z-20">
          <div className="flex items-center gap-4">
            ...
            <div>
               <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded bg-${theme?.color}-50 text-${theme?.color}-700 mb-1 inline-block`}>
                  {theme?.name}
               </span>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8 no-scrollbar">
           <section>
              <div className="flex items-center gap-2 mb-2">
                ...
                <i className="fas fa-circle-info text-[10px] text-slate-300" title="Hover over status for definitions"></i>
              </div>
              <div className="flex gap-2">
                {(['Green', 'Yellow', 'Red'] as const).map(s => (
                  <div key={s} className="relative flex-1 group">
                    {/* Tooltip (Positioned BELOW to avoid header clipping) */}
           ...
           <section>
              <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Core Outcome Definition</label>
              <textarea 
                disabled={!canEdit}
                value={activeOutcome.description}
                onChange={(e) => setActiveOutcome({...activeOutcome, description: e.target.value})}
           ...
           <section className="space-y-4">
              ...
              {suggestions.length > 0 && (
                <div className="space-y-3 animate-in fade-in slide-in-from-top-4">
                   <p className="text-[9px] font-bold text-blue-600 uppercase tracking-widest">AI Recommendations:</p>
                   {suggestions.map((s, i) => (
                     <div key={i} className="bg-blue-50 border border-blue-100 p-4 rounded-xl flex justify-between items-start gap-4 shadow-sm">
                        <div className="flex-1">
                           ...
                           <p className="text-[10px] font-bold text-blue-800 mt-2 tracking-tight">Target: {s.threshold}</p>
                        </div>
                        <button 
                          onClick={() => handleAddSuggested(s)}
                          className="p-2 bg-white text-blue-600 rounded-lg hover:bg-blue-600 hover:text-white transition-all shadow-sm active:scale-90"
                        >
              ...
              <div className="space-y-4">
                 {outcomeMeasures.map(m => (
                    <div key={m.id} className="bg-white border border-slate-200 p-5 rounded-2xl shadow-sm space-y-3 group hover:border-blue-200 transition-colors">
                       <div className="flex justify-between items-start">
                          <div className="flex-1 min-w-0">
                             <h4 className="font-bold text-slate-900 truncate">{m.name}</h4>
                             <p className="text-xs text-slate-500 font-light mt-1 line-clamp-2">{m.definition}</p>
                          </div>
                          <div className="flex items-center gap-1 ml-4 shrink-0">
                             <span className="text-[10px] font-bold text-slate-400 bg-slate-100 px-2 py-1 rounded uppercase tracking-widest">{m.cadence}</span>
                             {canEdit && (
                 ...
                 {outcomeMeasures.length === 0 && !isSuggesting && suggestions.length === 0 && (
                   <div className="py-12 text-center bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl">
                      <p className="text-sm text-slate-400 italic">No measures defined for this outcome.</p>
                   </div>
                 )}
              </div>
           </section>

...
const OutcomeDetail: React.FC<OutcomeDetailProps> = ({ outcome, measures, currentUser, onClose, onUpdate, onAddMeasure, onUpdateMeasure, onDeleteMeasure, themes, users }) => {
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-10 no-scrollbar">
          <section>
            ...
            <div className="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-100">
              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 block">Primary Owner</label>
                <select
                  ...
                  value={activeOutcome.owner_user_ids[0] || ''}
                  disabled={!canEdit}
                  onChange={(e) => setActiveOutcome({ ...activeOutcome, owner_user_ids: [e.target.value] })}
                >
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 block">Supporting / Sponsor</label>
                <select
                  className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs font-bold text-slate-700 outline-none focus:ring-2 focus:ring-blue-500"
                  value={activeOutcome.support_sponsor_id || ''}
                  disabled={!canEdit}
                  onChange={(e) => setActiveOutcome({ ...activeOutcome, support_sponsor_id: e.target.value })}
                >
              ...
              {(['Green', 'Yellow', 'Red'] as const).map(s => (
                <div key={s} className="relative flex-1 group">
                  ...
                  <button
                    ...
                    className={`w-full py-3 rounded-xl border-2 font-bold transition-all ${activeOutcome.status === s

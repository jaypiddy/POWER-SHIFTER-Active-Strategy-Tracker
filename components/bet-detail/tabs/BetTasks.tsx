        ...
        <>
            <div className="space-y-6">
                <div className="flex justify-between items-center">
                    <h3 className="font-bold text-white">Bet Tasks</h3>
                    {canEdit && (
                        <motion.button
                            onClick={handleAddAction}
                            whileHover={{ scale: 1.05 }}
                            whileTap={{ scale: 0.95 }}
                            className="text-xs font-bold text-blue-400 bg-blue-900/30 px-3 py-1.5 rounded-lg hover:bg-blue-900/50 transition-colors shadow-sm border border-blue-900/50"
                        >
                    ...
                    <AnimatePresence mode="popLayout">
                        {bet.actions.map((action) => {
                            ...
                            return (
                                <motion.div
                                    ...
                                    key={action.id}
                                    id={`task-${action.id}`}
                                    className="bg-slate-900 rounded-xl border border-slate-800 p-5 space-y-4 transition-all hover:border-blue-500/30 shadow-sm relative group"
                                >
                                    <div className="flex justify-between items-start gap-4">
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2 mb-1">
                                                <p className="font-bold text-white truncate">{action.title}</p>
                                                <span className="text-[10px] font-bold text-slate-400 bg-slate-800 px-1.5 rounded border border-slate-700">{action.tshirt_size}</span>
                                            </div>
                                            <p className="text-xs text-slate-400 font-light line-clamp-1 mb-3">{action.description || 'No description provided.'}</p>
                                            <div className="flex items-center gap-6">
                                                ...
                                                <div className="flex-1 max-w-[100px] flex items-center gap-2">
                                                    ...
                                                    <span className="text-[10px] font-bold text-slate-400">{action.progress}%</span>
                                                </div>
                                                {assignee && (
                                                    <div className="flex items-center gap-1">
                                                        <img src={assignee.avatar} className="w-4 h-4 rounded-full" alt="" />
                                                        <span className="text-[10px] font-bold text-slate-400">{assignee.firstName}</span>
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                            {canEdit && (
                                                <button aria-label="Edit Task" className="text-slate-400 hover:text-blue-400 p-2 transition-colors">
                                                    <i className="fas fa-pen text-sm"></i>
                                                </button>
                                            )}
                                            {canEdit && (
                                                <button aria-label="Delete Task" className="text-slate-400 hover:text-rose-400 transition-colors p-2">
                        ...
                        {bet.actions.length === 0 && (
                            <motion.div
                                initial={{ opacity: 0 }}
                                animate={{ opacity: 1 }}
                                className="py-12 text-center bg-slate-900/50 border-2 border-dashed border-slate-800 rounded-2xl"
                            >
            ...
            {editingAction && (
                <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-[60] flex items-center justify-center p-4">
                    <div className="w-full max-w-lg bg-slate-900 rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 border border-slate-800">
                        <header className="px-6 py-4 border-b border-slate-800 flex justify-between items-center bg-slate-950/30">
                            <h3 className="text-sm font-bold text-slate-200 uppercase tracking-widest">Edit Strategic Task</h3>
                            <button onClick={() => setEditingAction(null)} className="text-slate-400 hover:text-white">
                        ...
                        <div className="p-8 space-y-6">
                            <div>
                                ...
                                <input
                                    type="text"
                                    className="w-full bg-slate-950 border border-slate-700 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 outline-none font-bold text-white placeholder:text-slate-600"
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    <select
                                        className="w-full bg-slate-950 border border-slate-700 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 outline-none font-bold text-white appearance-none cursor-pointer"
                            ...
                            <div>
                                ...
                                <p className="text-[9px] text-slate-500 mt-2 font-medium italic">Task progress increments are restricted to 25% steps.</p>
                            </div>

                            <div className="pt-4">
                                <button
                                    onClick={handleDoneEditing}
                                    className="w-full py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-500 transition-all shadow-xl shadow-blue-900/20"
                                >
                                    Done Editing
                                </button>
                            </div>
